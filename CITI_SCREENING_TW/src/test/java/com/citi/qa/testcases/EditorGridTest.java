...
/**
 * @author Chouhan_h Test the Editor functionality of kitchen-sink Home Application
 */
    ...
    extends PageObject
{
    ...
    private void waitForDivWithText( String text )
    {
        new WebDriverWait( this.webDriver, 600 ).until(
                ExpectedConditions.visibilityOfAllElementsLocatedBy( By.xpath( "//div[text()=\"" + text + "\"]" ) ) );
    }

    private void setCellValueByLabel( String text, int column, String value )
        throws InterruptedException
    {
        WebElement cell = this.webDriver.findElement(
                By.xpath( "//tr[td/div[text()=\"" + text + "\"]]/td[" + column + "]" ) );
        cell.click();
        new WebDriverWait( this.webDriver, 2 ).until(
                ExpectedConditions.visibilityOf( cell.findElement( By.xpath( "//input" ) ) ) );
    ...
    {
        WebElement cell = this.webDriver.findElement(
                By.xpath( "//tr[td/div[text()=\"" + label + "\"]]/td[" + column + "]" ) );
        cell.click();
        new WebDriverWait( this.webDriver, 2 ).until(
                ExpectedConditions.visibilityOf( cell.findElement( By.xpath( "//input" ) ) ) );
        cell.findElement( By.xpath( "//input" ) ).sendKeys( search );
        new WebDriverWait( this.webDriver, 2 ).until(
                ExpectedConditions.visibilityOf( cell.findElement( By.xpath( "//li[text()=\"" + value + "\"]" ) ) ) );
